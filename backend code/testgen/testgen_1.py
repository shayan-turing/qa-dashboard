...
def main(base_path: str = DEFAULT_BASE_PATH, run_tests: bool = True, interactive: bool = True) -> Dict[str, Any]:
    print(colored(f"[START] Running test generation on base_path: {base_path}", "cyan"))
    ...
    for path in files:
        tool_path = Path(path)
        file_name = tool_path.stem
        print(colored("=" * 66, "blue"))
        print(colored(f" Processing: {file_name}", "blue", attrs=["bold"]))
        print(colored("=" * 66, "blue"))
        ...
        for ttype in TEST_TYPES:
            print(colored(f"[INFO] Generating {ttype.upper()} tests for {file_name}", "yellow"))
            ...
            print(colored(f"[RESULT] {file_name} ({ttype}): {tool_results[ttype]}", "green" if success else "red"))

        per_tool_summary.append({"tool": file_name, "results": tool_results})

    print(colored("[DONE] All test generations complete.", "cyan"))
